...
function ProductModal({
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const url = await uploadProductImage(file, form.code || 'temp');
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        >
          ...
          <div className="mb-5 p-4 bg-gray-50 dark:bg-slate-700/50 rounded-xl border border-gray-200 dark:border-slate-600">
            <p className="text-xs font-semibold text-gray-700 dark:text-slate-300 mb-2 flex items-center gap-1">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center">
              ...
              <div className="flex-1 w-full">
                ...
                <button
                  ...
                  disabled={uploading}
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#0A1B2A] hover:bg-[#2563EB] dark:bg-slate-600 dark:hover:bg-slate-500 text-white text-sm font-medium px-4 py-2 rounded-lg disabled:opacity-60"
                >
                  {uploading ? <><Loader2 className="w-4 h-4 animate-spin" /> Subiendo...</> : <><Upload className="w-4 h-4" /> {form.imageUrl ? 'Cambiar imagen' : 'Subir imagen'}</>}
                </button>
                {form.imageUrl && (
                  <button
                    type="button"
                ...
                <p className="text-[10px] text-gray-500 dark:text-slate-400 mt-2">
                  Si no subes imagen, se generará una visualización con SVG. Formatos: JPG, PNG, WebP. Máx 2MB.
                </p>
                <div className="mt-2">
                  <label className="text-[10px] font-semibold text-gray-700 dark:text-slate-300">O pega una URL de imagen:</label>
                  <input
                    type="url"
                    value={form.imageUrl?.startsWith('data:') ? '' : (form.imageUrl || '')}
                    onChange={e => setForm({ ...form, imageUrl: e.target.value || undefined })}
                    className="w-full mt-1 px-2 py-1.5 border border-gray-300 dark:border-slate-600 rounded text-xs bg-white dark:bg-slate-700 dark:text-slate-100"
